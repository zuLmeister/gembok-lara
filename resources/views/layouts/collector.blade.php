...
<body class="bg-gray-50 min-h-screen" x-data="{ sidebarOpen: false }">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        ...
        <nav class="p-4 space-y-2">
            <a href="{{ route('collector.dashboard') }}" class="flex items-center px-4 py-3 text-gray-300 rounded-lg hover:bg-slate-700 {{ request()->routeIs('collector.dashboard') ? 'bg-slate-700 text-white' : '' }}">
        ...
        <header class="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-30">
            <div class="flex items-center justify-between px-4 py-3">
                ...
                <div class="flex items-center space-x-4">
                    <span class="text-gray-600">{{ Auth::user()->name ?? 'Collector' }}</span>
                    <div class="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center">
                        <i class="fas fa-hand-holding-dollar text-blue-600"></i>
                    </div>
                </div>
            </div>
        </header>
